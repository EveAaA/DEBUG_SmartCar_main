...
void Navigation_Process_Image(float Target_Pos_X,float Target_Pos_Y)
{
    ...
    switch(Navigation_State)
    {
        ...
        case Move_State:
            ...
            {
                Navigation.Cur_Position_X = (FINETUNING_DATA.dx/10.0f)*0.85f+(Target_Pos_X - Get_X_Distance())*0.15f;//互补滤波
            }
            else
            {
            ...
            {
                Navigation.Cur_Position_Y = (FINETUNING_DATA.dy/10.0f)*0.85f+(Target_Pos_Y - Get_Y_Distance())*0.15f;//互补滤波
            }
            else
            {
