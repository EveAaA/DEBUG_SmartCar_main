...
{
    uint8_t data_high_8bit = data >> 8;
    uint8_t data_low_8bit  = data & 0xff;
    g_dataWrite[startAddr] = data_low_8bit;
    g_dataWrite[startAddr + 1] = data_high_8bit;
...
{
    if (kStatus_Success != SD_ReadBlocks(&g_sd, g_dataWrite, DATA_BLOCK_START, 1U))
    {
        LED_RED(LED_ON);
        return 0;
    }
    ...
    int16_t data = ((g_dataWrite[startAddr] >> 7) == 0) ? ((g_dataWrite[startAddr] + (g_dataWrite[startAddr + 1] << 8))) : (-(65536 - (g_dataWrite[startAddr] + (g_dataWrite[startAddr + 1] << 8))));
    return data;
}
...
{
    // 清空缓存区数据
    memset(g_dataWrite, 0x00, sizeof(g_dataWrite));
    // 从sd卡读取数据写入变量
    Brightness         = sd_read_data(ADDRESS(0));
    BorderCenter.x     = sd_read_data(ADDRESS(1));
    BorderCenter.y     = sd_read_data(ADDRESS(2));
    SmallPlaceCenter.x = sd_read_data(ADDRESS(3));
    SmallPlaceCenter.y = sd_read_data(ADDRESS(4));
    BigPlaceCenter.x   = sd_read_data(ADDRESS(5));
