...
UART_TAIl = b"\xfc\xbf"
martrixH = [[2.26415, 0.0, -2.26415], [-2.429171e-07, 2.307692, 2.429169e-07], [-1.237078e-09, -1.222659e-09, 1]]
...
def getTrueDiff():
    ...
    while True:
        ...
        if object:
            ...
            img.draw_line(160, 240, 160, 0, color = (255, 0, 0), thickness = 1)
    ...
    if object:
        ...
        cx = x1 + w / 2
        cy = y1 + h / 2
        center = [(cx - 160), (137 - cy), 1]
        np_center = np.array([[p] for p in center])#升维
        np_H = np.array(martrixH)
        Point = np_H * np_center
        worldDx, worldDy = int(Point[0][0] + 15), int(Point[1][0])
        worldDz = 0
        print(worldDx, worldDy)
        img.draw_cross(int(center[0] + 160), int(137 - center[1]), color = (0, 255, 255))
        img.draw_rectangle(x1, y1, w, h, color = (255, 0 ,0), thickness = 3)
