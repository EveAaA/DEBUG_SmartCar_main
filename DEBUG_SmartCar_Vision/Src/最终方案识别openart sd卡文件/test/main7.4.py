...
#        if offsetX == 0 and offsetY == 0:
        ...
        if object:
            ...
            if offsetX == 0 and offsetY == 0:
                ...
                if not IS_CARD_FLAG and isBesideRoad:
                    if scores > 0.70 and ErrBesideRoad > CY_ISBORDER_THRESHOLD and isBesideRoad:
                        return False
    ...
    if object and scores > 0.70:
        ...
        cx = x1 + w / 2
        cy = y1 + h / 2
        center = [(cx - 163 + offsetX), (162 - cy + offsetY), 1]
        ...
        worldDz = 0
        img.draw_line(0, 162, 320, 162, color = (255, 255, 0))
...
while(True):
    if test == True:
        while True:
#            getTrueDiff(offsetX = 39, offsetY = -48)
 #            getTrueDiff()
#             getVolumeUpErr()
#            img = sensor.snapshot()
#            classify(img)
