...
borderOrNot = '/sd/BorderOrNot.tflite'
net2 = '/sd/Debug_VisionPlace.tflite'
labels = [line.rstrip("\n") for line in open("/sd/label.txt")] + ["none"]
label =  [line.rstrip("\n") for line in open("/sd/border.txt")] + ["none"]
#载入模型
...
def classifyAlphaBet(img, minRect):
    ...
    inputImage.erode(1)
    model = tf.load(borderOrNot, load_to_fb=True)
...
while(True):
    ...
    if obj:
        ...
        if(scores > 0.70):
            ...
            delta = math.atan((cx - center_x) / (240 - cy)) * 180 / math.pi
            print(delta)
            # NOT_BORDER += classifyAlphaBet(img, tuple(minRect))
            if cy >= TARGET_LINE and IS_OBJ >= 5 :
